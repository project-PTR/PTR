...
const urlSearch = "http://localhost:8080/searchNotice";
...
function getNotice() {
  document.querySelector(".head_subtitle").textContent =
    "홈 > 고객센터 > 공지사항";
...
function toggleFontWeight() {
  ...
  noticeBtn.classList.contains("bold")
    ? noticeBtn.classList.remove("bold")
    : noticeBtn.classList.add("bold");
  const qnaBtn = document.querySelector(".qna_btn");
  qnaBtn.classList.contains("bold")
    ? qnaBtn.classList.remove("bold")
    : qnaBtn.classList.add("bold");
}
...
document
  .querySelector(".content_body_page_left")
...
document
  .querySelector(".content_body_page_right")
  .addEventListener("click", () => {
